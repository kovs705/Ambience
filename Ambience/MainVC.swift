...
extension MainVC: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let ambiCell = collectionView.dequeueReusableCell(withReuseIdentifier: AmbiCell.id, for: indexPath) as? AmbiCell else {
            return UICollectionViewCell()
        }

        if let ambience = presenter.ambiences?.all[indexPath.row] {
            ambiCell.configure(ambience: ambience)
            ambiCell.isHighlighted = false
        }
